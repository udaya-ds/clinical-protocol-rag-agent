...
def score_case(expected: dict, actual: dict) -> dict:
    ...
    for field in ["protocol_number", "phase", "indication", "study_design",
        ...
        if exp_val is None:
            # expected=None means "we haven't verified the true value for
            # this field," NOT "the true value should be null." Confirmed
            # directly: AZ's actual extraction of number_of_sites=100 and
            # treatment_duration_weeks=4 were BOTH independently verified
            # correct against the source text, but scored as MISMATCH here
            # purely because the ground truth had simply never been filled
            # in for those fields. Penalizing a correct extraction for an
            # unverified field is a scoring bug, not a real extraction
            # problem - skip fields we have no ground truth for instead of
            # auto-failing them.
            results[field] = {"expected": exp_val, "actual": act_val,
                               "match": True, "skipped_unverified": True}
            continue
...
def summarize(all_results: dict) -> dict:
    ...
    for case_results in all_results.values():
        for field in scalar_fields:
            if case_results[field].get("skipped_unverified"):
                skipped += 1
                continue  # don't count toward accuracy either way - no ground truth to check against
            total += 1
        ...
        if case_results["primary_endpoint"]["match"]:
            correct += 1
    summary = {"scalar_field_accuracy": f"{correct}/{total} ({100*correct/total:.1f}%)" if total else "n/a"}
    if skipped:
        summary["unverified_fields_skipped"] = skipped
    return summary
    ...
    for case in eval_set["cases"]:
        ...
        for field, r in result.items():
            if field in ("treatment_arms", "eligibility"):
                ...
                if r.get("skipped_unverified"):
                    status = "SKIPPED (no ground truth)"
                else:
